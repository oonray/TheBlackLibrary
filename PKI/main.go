...
func NewPKI(org string, addr string, code string) (*PKI, error) {
	...
	out.ca, err := x509.CreateCertificate(
		rand.Reader,
		out.cert,
		out.cert,
		out.pub,
...
func (pki *PKI) Save(path string) error {
	out, err := os.Create(fmt.Sprintf("%s/%s", path, "cert.pem"))
	if err != nil {
		return err
	}
	...
	out, err = os.Create(fmt.Sprintf("%s/%s", path, "key.pem"), os.O_WERONLY|os.O_CREATE|os.O_TRUNK, 0600)
	if err != nil {
		return err
	}
	...
	out, err = os.Create(fmt.Sprintf("%s/%s", path, "pub.pem"), os.O_WERONLY|os.O_CREATE|os.O_TRUNK, 0600)
	if err != nil {
		return err
	}
